...
public class GalleryDao {
	...
	public List<GalleryVo> getGalleryList(Connection conn) throws Exception {
		...
		// SQL
		String sql = "SELECT A.THUMBNAIL_YN, G.NO AS GALLERY_NO , G.TITLE , G.WRITER_NO , G.HIT , G.ENROLL_DATE , G.MODIFY_DATE , G.STATUS , A.NO              AS ATTACHMENT_NO , A.REF_NO , A.ORIGIN_NAME , A.CHANGE_NAME , A.FILE_PATH , M.NICK AS WRITER_NICK FROM GALLERY G JOIN MEMBER M ON G.WRITER_NO = M.NO JOIN ATTACHMENT A ON G.NO = A.REF_NO WHERE G.STATUS = 'O' ORDER BY G.NO DESC";
		...
		while(rs.next()) {
			...
			GalleryVo galleryVo = new GalleryVo(galleryNo, title, writerNo, hit, enrollDate, modifyDate, status, attachmentNo, refNo, originName, changeName, filePath, thumbnailYn, writerNick);
