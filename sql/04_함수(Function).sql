...
FROM DUAL;  --DUAL: 한개의 행이나 열만 가져옴

-- EMPLOYEE 테이블에서 모든 사원의 이메일, 이메일 글자 수 (글자 수 별칭) 조회

SELECT EMAIL, LENGTH(EMAIL) AS "글자 수"
FROM EMPLOYEE;

  
/*
    2) INSTR
        - INSTR(칼럼|'문자값', '문자'[, POSITION[, OCCURRENCE]]) //대괄호 안에 있는 것들은 선택해도 안해도 되는 것들
        - 지정한 위치부터 지정된 숫자 번째로 나타나는 문자의 시작 위치를 반환한다.
*/
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL; -- 첫번째 인자에서 두번째 인자를 탐색
SELECT INSTR('AABAACAABBAA', 'B', 3) FROM DUAL; -- 3번째 인자값 인덱스 부터 탐색 시작
SELECT INSTR('AABAACAABBAA', 'B', 1, 2) FROM DUAL; -- (4번째 인자값) 번 째 등장한 값의 위치
SELECT INSTR('AABAACAABBAA', 'B', -1) FROM DUAL; -- -1은 뒤에서 첫번째 부터 탐색을 의미
SELECT INSTR('AABAACAABBAA', 'B', -1, 3) FROM DUAL; -- 뒤에서 세번째 탐색을 의미

/*
    3) LPAD / RPAD
        - LPAD/RPAD(컬럼|'문자값', 길이(글자개수)[, '덧붙이려고 하는 문자'])
        - 제시된 컬럼|'문자값'에 임의의 문자를 왼쪽 또는 오른쪽에 덧붙여 최종 N 길이 만큼의 문자열을 반환한다.
        - 문자에 대해 통일감 있게 표시하고자 할 때 사용한다.
*/
 
SELECT LPAD(Email,20, 'X')
FROM EMPLOYEE
;

/*
     4)LTRIM/RTRIM(컬럼|'문자값'[, '제거하고자 하는 문자'])
...
SELECT TRIM(BOTH 'Z' FROM 'ZZZKHZZZ') FROM DUAL; -- default값
...
/*
    7) LOWER / UPPER / INITCAP //대소문자, CAPITAL
...
ALTER SESSION SET NLS_DATE_FORMAT = 'RR/MM/DD'; --RR은 1950~ 2049연도 까지
...
SELECT TO_CHAR(1234, '999999') AS HELLO FROM DUAL; -- 6칸의 공간을 확보, 오른쪽 정렬, 빈칸은 공백으로 채운다.
...
SELECT TO_CHAR(SYSDATE, 'MON DY, YYYY') FROM DUAL; --포멧 인자에 DY OR DAY를 입력하면 한글로 요일 반환
...
SELECT TO_DATE('20211014', 'YYYY/MM/DD') FROM DUAL;
...
SELECT EMP_NAME, BONUS, (SALARY * 12) + (SALARY * NVL(BONUS, 0))
FROM EMPLOYEE;
-- EMPLOYEE 테이블에서 사원명, 부서 코드 조회(단, 부서코드가 NULL이면 "부서없음" 출력
SELECT EMP_NAME, NVL(DEPT_CODE, '부서없음') 부서코드
FROM EMPLOYEE;

-- 보너스 일괄적용 하여 연봉 조회(있던 사람들은 무조건 10% 적용, 없으면 안줌)
-- 사원이름, 보너스, 연봉
SELECT EMP_NAME, BONUS, SALARY * 12 + SALARY * NVL2(BONUS, 0.1, 0) 연봉, SALARY, SALARY * NVL2(BONUS, 0.1,0)
FROM EMPLOYEE;
...
SELECT 123 AS SAJDFLKDSJFKLSDJFLK FROM DUAL;
...
--사원 이름, 주민번호, 성별
SELECT EMP_NAME, EMP_NO, DECODE(SUBSTR(EMP_NO, 8, 1), '1', 1, '2', 2) 성별
FROM EMPLOYEE;
...
--그룹함수는 NULL값을 취급하지 않는다. NULL값을 치환한 뒤 계산