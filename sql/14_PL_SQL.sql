...
DECLARE
    ABC NUMBER;
    SSS VARCHAR2(100);
    P   CONSTANT NUMBER := 3.14;  --CONSTANT : 재할당 불가능하게 하는 함수(자바의 FINAL)
BEGIN
    ABC := 123;
    SSS := '홍길동';
...
BEGIN
    ...
    IF MONEY >= 8000000
        THEN DBMS_OUTPUT.PUT_LINE('고급');
    ELSIF MONEY >= 5000000
        THEN DBMS_OUTPUT.PUT_LINE('중급');
    ELSE
        DBMS_OUTPUT.PUT_LINE('초급');
    END IF;
END;
/
...
BEGIN
    X := 10;
    CASE X
        WHEN 3 THEN DBMS_OUTPUT.PUT_LINE('3입니다~');
        WHEN 10 THEN DBMS_OUTPUT.PUT_LINE('10입니다~');
        ELSE DBMS_OUTPUT.PUT_LINE('모름~');
    END CASE;
END;
/
...
EXCEPTION
    WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('THIS PARAGRAPH HAVE A ERROR! HURRY UP! VIRUS IS COMING..');
END;
/
